...
async function sendFundingNotification(loan: any, lenderId: number) {
  ...
  // ========================================================================
  // ENCRYPTED KEY VAULT ROUTES (Server-side passphrase-encrypted key storage)
  // ========================================================================

  // Store encrypted key on server
  app.post("/api/key-vault", authenticateToken, async (req, res) => {
    try {
      const userId = (req as any).user.id;
      const { loanId, role, publicKey, encryptedPrivateKey, iv, salt } = req.body;
      
      if (!loanId || !role || !publicKey || !encryptedPrivateKey || !iv || !salt) {
        return res.status(400).json({ message: "Missing required fields" });
      }
      
      // Verify user is authorized for this loan/role
      const loan = await storage.getLoan(loanId);
      if (!loan) {
        return res.status(404).json({ message: "Loan not found" });
      }
      ...
      if (role === 'borrower' && loan.borrowerId !== userId) {
        return res.status(403).json({ message: "Not authorized as borrower for this loan" });
      }
      if (role === 'lender' && loan.lenderId !== userId) {
        return res.status(403).json({ message: "Not authorized as lender for this loan" });
      }
      
      const keyRecord = await storage.storeEncryptedKey({
        loanId,
        userId,
        role,
        publicKey,
      ...
      console.log(`[KeyVault] Stored encrypted key for user ${userId}, loan ${loanId}, role ${role}`);
      res.json({ success: true, id: keyRecord.id });
    } catch (error) {
      console.error("[KeyVault] Error storing encrypted key:", error);
      res.status(500).json({ message: "Failed to store encrypted key" });
    }
  });

  // Get encrypted key from server
  app.get("/api/key-vault/:loanId/:role", authenticateToken, async (req, res) => {
    try {
      const userId = (req as any).user.id;
      const loanId = parseInt(req.params.loanId);
      ...
      const keyRecord = await storage.getEncryptedKey(loanId, userId, role);
      if (!keyRecord) {
        return res.status(404).json({ message: "No encrypted key found" });
      }
      ...
      console.error("[KeyVault] Error retrieving encrypted key:", error);
      res.status(500).json({ message: "Failed to retrieve encrypted key" });
    }
  });

  // Check if encrypted key exists
  app.get("/api/key-vault/:loanId/:role/exists", authenticateToken, async (req, res) => {
    try {
      const userId = (req as any).user.id;
      const loanId = parseInt(req.params.loanId);
      const role = req.params.role;
      ...
      const keyRecord = await storage.getEncryptedKey(loanId, userId, role);
      res.json({ exists: !!keyRecord, publicKey: keyRecord?.publicKey });
    } catch (error) {
      console.error("[KeyVault] Error checking key existence:", error);
      res.status(500).json({ message: "Failed to check key existence" });
    }
  });
